...
def align_LEXICON(example, lexicon_tokens):
    input_tokens = example['toxic_word_tokens']
    token_classes = [0] * 128
    
    for lt in lexicon_tokens:
        res = find_overlap_position(input_tokens, lt)
        if res != -1:
            token_classes[res:res+len(lt)] = [1] * len(lt)
...
class CreateDataset:
    ...
    def main(self):
        ...
        for lang in self.langs:
            ...
            mpd = mpd.map(align_LEXICON, batched=False, fn_kwargs={'lexicon_tokens': lexicon_token_list})
        ...
        combined_dataset.save_to_disk('dataset_test_v1')
        print("Dataset creation complete.")


# python create_dataset_all.py --langs am ar de en es fr he hi hin it ja tt kk zh ru uk
